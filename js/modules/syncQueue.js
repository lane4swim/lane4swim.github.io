...
function renderView(container, queue) {
  ...
  wrap.appendChild(el('div', { class: 'page-head' }, [
    ...
    el('div', { class: 'page-actions' }, [
      ...
      el('button', { class: 'btn btn-primary', disabled: pending + errored === 0, onclick: () => runSimulatedSync(refresh) }, t('syncqueue.syncButton')),
    ]),
  ]));
...
async function runSimulatedSync(onDone) {
  ...
  for (const evt of toSend) {
    ...
    if (willFail) {
      await updateSyncEvent(evt.id, { status: 'error', attempts: (evt.attempts || 0) + 1, lastError: t('syncqueue.simulatedError') });
    } else {
  ...
  toast(stillFailing > 0 ? t('syncqueue.syncDoneFailed', { count: stillFailing }) : t('syncqueue.syncDoneOk'), stillFailing > 0 ? 'error' : 'info');
  onDone?.();
}
