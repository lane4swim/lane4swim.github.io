...
function renderList(container, sessions, groups, athletes, plans) {
  ...
  wrap.appendChild(el('div', { class: 'page-head' }, [
    el('div', {}, [el('div', { class: 'page-eyebrow' }, `${sessions.length} Einheiten`), el('h1', { class: 'mt-0' }, 'Trainingseinheiten & Feedback')]),
    el('div', { class: 'page-actions' }, [el('button', { class: 'btn btn-primary', onclick: () => openSessionModal(null, groups, athletes, refresh) }, '+ Einheit erfassen')]),
  ]));
  ...
  if (sessions.length === 0) { host.appendChild(emptyState('Noch keine Einheiten', 'Erfasse Anwesenheit und RPE-Feedback nach jedem Training.', null)); return; }
  const table = el('table');
  table.appendChild(el('thead', {}, el('tr', {}, [el('th', {}, 'Datum'), el('th', {}, 'Gruppe'), el('th', {}, 'Anwesend'), el('th', {}, 'Ø RPE'), el('th', {}, '')])));
  ...
  sessions.slice().sort((a, b) => b.date.localeCompare(a.date)).forEach(s => {
    ...
    tbody.appendChild(el('tr', { class: 'row-click', onclick: () => navigate('sessions', s.id) }, [
      ...
      el('td', {}, el('button', { class: 'btn btn-ghost btn-sm', onclick: (e) => { e.stopPropagation(); navigate('sessions', s.id); } }, 'Öffnen')),
    ]));
  });
...
async function renderDetail(container, sessionId) {
  ...
  if (!session) { container.appendChild(emptyState('Nicht gefunden', 'Diese Einheit existiert nicht mehr.', el('button', { class: 'btn btn-primary', onclick: () => navigate('sessions') }, 'Zurück'))); return; }
  const group = groups.find(g => g.id === session.groupId);
  const plan = plans.find(p => p.id === session.planId);

  const wrap = el('div');
  wrap.appendChild(el('button', { class: 'btn btn-ghost btn-sm mb-16', onclick: () => navigate('sessions') }, '← Alle Einheiten'));
  wrap.appendChild(el('div', { class: 'page-head' }, [
    el('div', {}, [el('div', { class: 'page-eyebrow' }, group?.name || 'Ohne Gruppe'), el('h1', { class: 'mt-0' }, fmtDateLong(session.date))]),
    el('div', { class: 'page-actions' }, [
      el('button', { class: 'btn btn-ghost', onclick: () => openSessionModal(session, groups, athletes, () => { clear(container); renderDetail(container, sessionId); }) }, 'Bearbeiten'),
      el('button', { class: 'btn btn-danger', onclick: () => confirmAction('Diese Trainingseinheit löschen?', async () => { await remove('sessions', sessionId); toast('Einheit gelöscht'); navigate('sessions'); }) }, 'Löschen'),
    ]),
  ]));
  wrap.appendChild(laneWave());
  if (plan) wrap.appendChild(el('p', {}, `Basierend auf Plan: ${plan.name}`));
  if (session.trainerNote) wrap.appendChild(el('div', { class: 'card' }, [el('h3', { class: 'mt-0' }, 'Trainernotiz'), el('p', {}, session.trainerNote)]));

  const attCard = el('div', { class: 'card' }, [el('h3', { class: 'mt-0' }, 'Anwesenheit & Feedback')]);
  (session.attendance || []).forEach(rec => {
    ...
    attCard.appendChild(el('div', { class: 'list-row' }, [
      ...
      el('div', { style: 'flex:1' }, [
        ...
        el('div', { class: 'text-slate text-sm' }, rec.note || (rec.present ? '' : 'Fehlgrund nicht angegeben')),
      ]),
      rec.present ? badge(rec.rpe ? `RPE ${rec.rpe}` : 'anwesend', 'done') : badge('abwesend', 'open'),
    ]));
  });
...
async function renderAthleteView(container) {
  ...
  const wrap = el('div');
  wrap.appendChild(el('div', { class: 'page-head' }, [el('div', {}, [el('div', { class: 'page-eyebrow' }, 'Mein Training'), el('h1', { class: 'mt-0' }, 'Meine Trainingseinheiten')])]));
  wrap.appendChild(laneWave());
  if (!me) { wrap.appendChild(el('p', {}, 'Kein Athletenprofil verknüpft.')); container.appendChild(wrap); return; }
  const mySessions = sessions.filter(s => s.attendance?.some(a => a.athleteId === me.id)).sort((a, b) => b.date.localeCompare(a.date));
  if (mySessions.length === 0) { wrap.appendChild(emptyState('Keine Einheiten', 'Für dich wurden noch keine Trainingseinheiten erfasst.', null)); container.appendChild(wrap); return; }
  const table = el('table');
  table.appendChild(el('thead', {}, el('tr', {}, [el('th', {}, 'Datum'), el('th', {}, 'Anwesenheit'), el('th', {}, 'RPE'), el('th', {}, 'Notiz')])));
  const tbody = el('tbody');
  mySessions.forEach(s => {
    const rec = s.attendance.find(a => a.athleteId === me.id);
    tbody.appendChild(el('tr', {}, [el('td', {}, fmtDateLong(s.date)), el('td', {}, rec.present ? badge('anwesend', 'done') : badge('abwesend', 'open')), el('td', {}, rec.rpe || '—'), el('td', {}, rec.note || '—')]));
  });
  table.appendChild(tbody);
...
function openSessionModal(session, groups, athletes, onSaved) {
  ...
  form.appendChild(el('div', { class: 'form-grid' }, [field('Datum', fDate), field('Gruppe', fGroup)]));
  form.appendChild(field('Trainernotiz', fNote, { hint: 'Allgemeine Beobachtung zur Einheit' }));

  const attWrap = el('div', { class: 'field' });
  attWrap.appendChild(el('label', {}, 'Anwesenheit & RPE (1–10)'));
  ...
  function drawAttendance() {
    ...
    data.attendance.forEach(rec => {
      ...
      const noteInput = el('input', { type: 'text', value: rec.note || '', placeholder: 'Notiz', style: 'flex:1', oninput: (e) => rec.note = e.target.value });
  ...
  form.appendChild(el('div', { class: 'form-actions' }, [
    ...
    el('button', { type: 'submit', class: 'btn btn-primary' }, isEdit ? 'Speichern' : 'Erfassen'),
  ]));
  form.addEventListener('submit', async (e) => {
    ...
    toast(isEdit ? 'Änderungen gespeichert' : 'Einheit erfasst');
    close(); onSaved?.();
  });
  const { close } = openModal({ title: isEdit ? 'Einheit bearbeiten' : 'Trainingseinheit erfassen', bodyNode: form, wide: true });
}
